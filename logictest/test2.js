...
function counter(arr) {
    ...
    arr.forEach (function (num, i) {
        ...
        // arrUnique.push({id: num, count: counts[num]});
        arrTemp.push({[num]: counts[num]});
    });
    
    // arrUnique = arrUnique.filter((value, index, self) => index === self.findIndex((t) => (t.id === value.id && t.count >= value.count)));
    
    // arrUnique.forEach (function (num, i) {
    //     arrTemp.push({[num.id]: num.count});
    // });
...
// Set counts to {}
// Set arrTemp to []
// Foreach num inside arr
    // Set counts[num] to counts[num] ? counts[num] + 1 : 1
    // push {[num]: counts[num]} to arrTemp
//End For
// Return arrTemp
// End
